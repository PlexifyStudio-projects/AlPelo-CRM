...
const CheckoutModal = ({ appointment, onClose, onCompleted }) => {
  ...
  const handleSubmit = useCallback(async () => {
    ...
    try {
      const payload = {
        appointment_id: appointment._apt_id || (appointment._order_id ? null : appointment.id),
          ...
          <div className={`${b}__receipt-upload`}>
            {receiptFile ? (() => {
              ...
              <label className={`${b}__receipt-drop`}>
                <input type="file" accept="image/*,.pdf,.doc,.docx,.xls,.xlsx,.csv,.txt" onChange={(e) => setReceiptFile(e.target.files?.[0] || null)} hidden />
                <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round">
                  <path d="M21 15v4a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2v-4" />
                  <polyline points="17 8 12 3 7 8" />
                  <line x1="12" y1="3" x2="12" y2="15" />
                </svg>
                <span>Subir comprobante (foto, PDF, Word, Excel...)</span>
              </label>
            )}
          </div>
        </div>
